...
| `automation.legacyFallback` | لا | `never` | يحدد ما إذا كان مسموحاً لـ Astur العودة لاستخدام أدوات المنصة القديمة في حال فشل الوكيل الأصلي. تُضبط على `on-agent-failure` تلقائياً إذا كان المحرك `auto`. |
