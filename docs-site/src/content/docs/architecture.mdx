...
<div class="architecture-map" aria-label="Astur high-level architecture">
  <div class="architecture-stage">
    <div class="flow-node primary">User test<span>@astur-mobile/test</span></div>
...
- Flutter (Android): when a Flutter APK is detected, Astur reads the live widget tree through the **Dart VM service** instead of UIAutomator, while ADB still handles lifecycle and gestures. See [Frameworks: Flutter & React Native](./frameworks/).
...
<div class="astur-flow compact" aria-label="Astur inspector codegen flow">
  ...
  <div class="flow-arrow">→</div>
  <div class="flow-node primary">@astur-mobile/test code</div>
</div>
