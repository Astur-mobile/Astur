...
تقود `device.webContext()` واجهة WebView **داخل** تطبيقك. أما لاختبار *موقع ويب*
داخل متصفح الجهاز نفسه — Chrome على Android و Safari على iOS — فاستخدم
`device.browser`؛ راجع [الويب على الجوال](../mobile-web/). وما دون مستوى الصفحة
فالآلية واحدة، وما يضيفه المتصفح هو الهدف والتنقل.
