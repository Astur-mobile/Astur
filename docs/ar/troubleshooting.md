...
**الحل:** 
قم بتوفير `packageName` صراحةً في إعداداتك، أو تأكد من توفر أداة `aapt` ليتمكن Astur من استخراج هذا الاسم تلقائياً من ملف הـ APK.
...
- افتح قفل הـ iPhone ووافق على رسالة "الوثوق بهذا الكمبيوتر".
...
### سلسلة مفاتيح جهاز iOS החقيقي مقفلة (Keychain Locked)
...
- تجنب تشغيل הـ Inspector من واجهة طرفية تمنع انبثاق واجهة المستخدم لإدخال كلمة المرور.
- في أنظمة CI، قم باستيراد الشهادة إلى سلسلة مفاتيح مؤقتة ومفتوحة قبل التشغيل.

### جسر جهاز iOS החقيقي لا يستطيع التسجيل
...
- يُفضل إبقاء الجهاز متصلاً عبر כابل USB للاستفادة من نفق Xcode/CoreDevice.
- ألغِ تعيين `ASTUR_IOS_AGENT_HOST` إذا كان يجبر الاتصال عبر شبكة מحلية مقيدة.
- تجنب استخدام شبكات VPN أو מيزات عزل الشبكة (Network Isolation) بين الكمبيوتر والهاتف.
...
## مشاكل הـ Inspector وشجرة الواجهة

### הـ Inspector لا يصبح جاهزاً أبداً
**الأعراض:** يفتح הـ Inspector، لكن عجلة التحميل تستمر بالدوران إلى الأبد والشارة تعرض `Connecting…`. شجرة الواجهة فارغة.

**التحقق والحلول:**
- **تبويب متصفح قديم:** כל تشغيل جديد לـ `codegen` يفتح تبويباً جديداً ויستخدم منفذاً מختلفاً. إذا حاولت استخدام تبويب قديم من جلسة سابقة، فلن يتصل أبداً. أغلق التبويبات القديمة وركز على التبويب الجديد.
- **تطبيق متبقّي يعيق الاتصال:** عادةً يُنظف Astur الجلسات السابقة، لكن إن تعطلت هذه الميزة، ابحث عن عملية XCUITest عالقة وأنهها يدوياً:
  ...
  ```
- **حالة التطبيق:** إذا كان הـ Inspector يتصل لكن لا تظهر الشجرة، فتأكد أن التطبيق ليس في حالة אי-خمول (Non-idle)، مثل تشغيل فيديو مستمر أو رسوم متحركة לא-منتهية (راجع قسم "أداء iOS واستقراره").

### شجرة الواجهة فارغة في הـ Inspector على iOS
...
**الحل:** (نفس حلول הـ Android أعلاه، مع استخدام `ASTUR_IOS_AGENT_ENDPOINT`).
...
- تحقق من أن نقطة النهاية תدعم بروتوكول HTTP POST لمغلفات الأوامر.
